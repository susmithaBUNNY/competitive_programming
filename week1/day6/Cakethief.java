...
public class Solution {
    ...
    public static long maxDuffelBagValue(final CakeType[] cakeTypes,final int weightCapacity) {

        // calculate the maximum value that we can carry
        final int[] maxValueAtCapacity = new int[weightCapacity + 1];
    
        for (int i = 0; i <= weightCapacity; i++) {
            int currentMaxValue = 0;
            for (int j = 0; j < cakeTypes.length; j++) {
                final CakeType cake = cakeTypes[j];
                if (cake.weight == 0 && cakeTypes.length==1){
